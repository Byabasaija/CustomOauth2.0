$(document).ready(function() {
    ...
    if (localStorage.getItem('x-auth')) {
        if (window.location.search.length != 0) {
            $("#div_register , #div_oauth").toggleClass("centered-form__form");
            $("#div_register , #div_oauth").toggleClass("centered-form__form__hidden");
        } else {
...
jQuery('#register-form').on('submit', function(e) {
    ...
    xhttp.open("POST", window.location.origin + "/api/user/register", true);
